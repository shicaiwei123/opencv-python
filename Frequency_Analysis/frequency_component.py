...
def fre_phase_analysis(img, display=False):
    ...
    phase_spectrum_normal = np.uint8(255 * (phase_spectrum - np.min(phase_spectrum)) / (
            np.max(phase_spectrum) - np.min(phase_spectrum)))
    time_end = time.time()
...
if __name__ == '__main__':
    img_path = "/home/shicaiwei/project/opencv-python/data/7787-0.jpg"
